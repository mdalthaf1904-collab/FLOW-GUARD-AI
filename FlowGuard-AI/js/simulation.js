...
const SimulationEngine = (function() {
  ...
  function simulatePlan(approaches, greenAllocation, config) {
    ...
    keys.forEach(k => {
      ...
      // Effective saturation flow: always computed as perLaneSatFlow × numLanes.
      // If app.saturationFlow is stored and already pre-multiplied (> defaultSatFlow, from analyzeApproaches),
      // use it directly. If it equals the global per-lane rate, it has NOT been multiplied — apply lanes.
      // If not present, fall back to defaultSatFlow × numLanes.
      const appSatFlow = parseFloat(app.saturationFlow);
      const satFlow = (!isNaN(appSatFlow) && appSatFlow > defaultSatFlow)
        ? appSatFlow                   // already pre-multiplied (numLanes × satPerLane from analyzeApproaches)
        : defaultSatFlow * numLanes;   // compute correctly: per-lane rate × numLanes
